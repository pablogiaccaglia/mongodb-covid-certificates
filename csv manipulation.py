    ...
    for entry in peopleData:
        ...
        for _ in range(takes):
            ...
            row = {"PersonID":                      personID,
                   ...
                   'Result of the Test':            utils.getCovidTestResultCode(covidTest = test, isPositive = isPositive),
                   "Unique Certificate Identifier": utils.getRandomUniqueCertificateIdentifier(),
                   'Country of Testing':            'IT',

                   "Certificate valid from":        date + datetime.timedelta(days = utils.getCovidTestResultWaitDays(covidTest = test))
                                                    if not isPositive else None,

                   "Certificate valid until":       date + datetime.timedelta(days = utils.getCovidTestCertificateValidityDays(covidTest = test))
                                                    if not isPositive else None,
